...
- [ ] **Env metadata consumption (false-positive governance v1.1)**: never recommend adding env metadata merely because code *reads* an env var. Before any "add to metadata" suggestion, ALL four items must hold: ① a named platform consumer that requires the declaration, ② a concrete failure path when the var is missing (core function stops), ③ a minimal reproduction, ④ a contract link or code location proving the requirement. Missing any item → no auto-metadata change, report as INFO/REVIEW instead. When consuming checker findings (e.g. skill-release-audit JSON), quote them with code + location + classification reason intact — never re-summarize into a bare variable list; `ENV_OPTIONAL_OVERRIDE` / `ENV_RUNTIME_INJECTED` / `ENV_AMBIENT` findings must NOT be escalated to WARN unless an independent real failure path is found. Reverse direction: metadata declaring runtime-injected or ambient vars (e.g. `MULTICA_*`, `HOME`) should be flagged for removal, and declared-but-never-read vars flagged for confirm-then-remove.
